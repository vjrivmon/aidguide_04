...
# Inicializar ROS2 Galactic al inicio del script
echo -e "${YELLOW}Inicializando entorno ROS2...${NC}"
if [ -f "/opt/ros/galactic/setup.bash" ]; then
    source /opt/ros/galactic/setup.bash
    echo -e "${GREEN}Entorno ROS2 Galactic inicializado${NC}"
else
    echo -e "${RED}Error: No se encontró el archivo setup.bash de ROS2 Galactic.${NC}"
    echo -e "${YELLOW}Buscando otras versiones de ROS2...${NC}"
    
    # Buscar otras instalaciones de ROS2
    if [ -d "/opt/ros" ]; then
        ros_versions=$(ls /opt/ros)
        if [ -n "$ros_versions" ]; then
            echo -e "${YELLOW}Versiones de ROS2 encontradas: $ros_versions${NC}"
            ...
            if [ -f "/opt/ros/$ros_version/setup.bash" ]; then
                ...
                echo -e "${RED}Error: Versión no válida${NC}"
                exit 1
            fi
        else
            echo -e "${RED}Error: No se encontraron instalaciones de ROS2${NC}"
            exit 1
        fi
    else
        echo -e "${RED}Error: ROS2 no está instalado${NC}"
